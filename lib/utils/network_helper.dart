...
import 'package:air_menu_ai_app/utils/app_exception.dart';
...
class NetworkAPICall {
  ...
  static String baseUrl = "https://rastorant-server.vercel.app/v1/api/";
  ...
  Future<dynamic> patch(
    ...
    try {
      ...
      log('API body: ${jsonEncode(body)}'); // Log the JSON body

      final response = await httpClient
          .patch(
            Uri.parse(fullURL),
            body: jsonEncode(body), // <-- Encode body to JSON string
            headers:
                header ??
                {
    ...
    try {
      ...
      if (response.body.isNotEmpty) {
        try {
          ...
          log("JSON Decode Error in delete(): $e");
          return {"success": false, "message": response.body}; // Return as JSON
        }
      } else {
      ...
      log("API Error: $e");
      httpClient.close();
      return {"success": false, "message": "API call failed"}; // Return as JSON
    }
  }
